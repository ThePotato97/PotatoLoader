...
using PotatoLoader.LightJson;

#pragma warning disable 0168

namespace PotatoLoader.Installer
{
    internal static class Program
    {
        internal static bool ShouldCheckForUpdates = false;
        internal static string Title = "PotatoLoader Installer";
        ...
        private static readonly string[] foldersToCleanUp = new string[] { "PotatoLogs", "PotatoLoader" };
        ...
        private static void Main()
        {
            ...
            else
            {
                ...
                ManualZipPath = (Directory.GetCurrentDirectory() + "\\PotatoLoader.zip");
        ...
        private static void Install_NoGUI()
        {
            try
            {
                string version = null;
        ...
        private static void Install_GUI()
        {
        ...
        {
            SetDisplayText("Extracting PotatoLoader.zip...");
            Cleanup(dirpath);
        ...
        internal static void Cleanup(string dirpath)
        {
            ...
            {
                DialogResult result = MessageBox.Show($"PotatoLoader could not remove old files.{Environment.NewLine}Please close the game then click retry to try again.{Environment.NewLine}If issue persists please click cancel to dump logs.", Program.Title, MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
        ...
        {
            Directory.CreateDirectory(Path.Combine(dirpath, "PotatoLogs"));
            if (!Directory.Exists(Path.Combine(dirpath, "PotatoMods")))
                Directory.CreateDirectory(Path.Combine(dirpath, "PotatoMods"));
            if (!legacy_install && !selectedVersion.Equals("v0.2.2") && !Directory.Exists(Path.Combine(dirpath, "PotatoPlugins")))
                Directory.CreateDirectory(Path.Combine(dirpath, "PotatoPlugins"));
        }
        ...
        private static void Install_Legacy_02(string dirpath, string selectedVersion)
        {
            ...
            if (is_02)
            {
                string AssemblyGenerator_Folder = Path.Combine(Path.Combine(dirpath, "PotatoLoader"), "AssemblyGenerator");
                ...
                try { webClient.DownloadFileAsync(new Uri("https://github.com/ThePotato97/PotatoLoader/raw/master/BaseLibs/UnityDependencies/" + mainForm.UnityVersion + ".zip"), tempfilepath4); while (webClient.IsBusy) { } } catch (Exception ex) { run_fallback = true; }
                if (run_fallback)
                {
                    string subver = mainForm.UnityVersion.Substring(0, mainForm.UnityVersion.LastIndexOf("."));
                    JsonArray data = (JsonArray)JsonValue.Parse(Program.webClient.DownloadString("https://api.github.com/repos/ThePotato97/PotatoLoader/contents/BaseLibs/UnityDependencies")).AsJsonArray;
                    if (data.Count > 0)
                    {
                        ...
                        if (versionlist.Count > 0)
                        {
                            ...
                            webClient.DownloadFileAsync(new Uri("https://github.com/ThePotato97/PotatoLoader/raw/master/BaseLibs/UnityDependencies/" + latest_version + ".zip"), tempfilepath4);
                            while (webClient.IsBusy) { }
                        }
                    }
                }
