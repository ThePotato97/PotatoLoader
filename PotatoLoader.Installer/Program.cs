...
namespace PotatoLoader.Installer
{
    internal static class Program
    {
        ...
        private static readonly string[] foldersToCleanUp = new string[] { "Logs", "PotatoLoader" };
        ...
        private static void Install_NoGUI()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Installation failed; upload the created log to #PotatoLoader-support on discord", Title);
            ...
            {
                MessageBox.Show("Failed to Get Version List; upload the created log to #PotatoLoader-support on discord", Title);
                File.WriteAllText(Directory.GetCurrentDirectory() + $@"\MLInstaller_{DateTime.Now:yy-M-dd_HH-mm-ss.fff}.log", ex.ToString());
                Environment.Exit(-1);
            }
        }
