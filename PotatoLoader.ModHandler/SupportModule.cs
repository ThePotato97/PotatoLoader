...
namespace MelonLoader
{
    ...
    internal static class SupportModule
    {
        ...
        internal static void Initialize()
        {
            try
            {
                string basedir = Path.Combine(Path.Combine(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "PotatoLoader"), "Dependencies"), "SupportModules");
                string filepath = null;
                if (Imports.IsIl2CppGame())
                    filepath = Path.Combine(basedir, "PotatoLoader.Support.Il2Cpp.dll");
                else
                {
                    if (File.Exists(Path.Combine(Imports.GetAssemblyDirectory(), "PotatoLoader.CoreModule.dll")))
                        filepath = Path.Combine(basedir, "PotatoLoader.Support.Mono.dll");
                    else
                    {
                        if (IsOldUnity())
                            filepath = Path.Combine(basedir, "PotatoLoader.Support.Mono.Pre2017.2.dll");
                        else
                            filepath = Path.Combine(basedir, "PotatoLoader.Support.Mono.Pre2017.dll");
                    }
                }
                if (File.Exists(filepath))
                {
                    ...
                    if (data.Length > 0)
                    {
                        ...
                        {
                            type = assembly.GetType("PotatoLoader.Support.Main");
                            if (!type.Equals(null))
                            {
