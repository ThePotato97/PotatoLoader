    ...
    internal static class AssemblyGenerator
    {
        ...
        internal static void Check()
        {
            if (!Imports.IsIl2CppGame() || Initialize())
                ...
                PotatoLoaderBase.UNLOAD(false);
        }

        private static bool Initialize()
        {
            string GeneratorProcessPath = Path.Combine(Path.Combine(Path.Combine(Path.Combine(Imports.GetGameDirectory(), "PotatoLoader"), "Dependencies"), "AssemblyGenerator"), "PotatoLoader.AssemblyGenerator.exe");
            if (!File.Exists(GeneratorProcessPath))
            {
                MelonLogger.LogError("PotatoLoader.AssemblyGenerator.exe does not Exist!");
                return false;
            }
            var generatorProcessInfo = new ProcessStartInfo(GeneratorProcessPath);
            generatorProcessInfo.Arguments = $"\"{PotatoLoaderBase.UnityVersion}\" {"\"" + Regex.Replace(Imports.GetGameDirectory(), @"(\\+)$", @"$1$1") + "\""} {"\"" + Regex.Replace(Imports.GetGameDataDirectory(), @"(\\+)$", @"$1$1") + "\""} {(Force_Regenerate() ? "true" : "false")} {(string.IsNullOrEmpty(Force_Version_Unhollower()) ? "" : Force_Version_Unhollower())}";
