...
namespace Harmony
{
	public static class MethodPatcher
	{
		...
		{
			return CreatePatchedMethod(original, ("PotatoLoader_HARMONY_PATCH_" + PotatoLoader.BuildInfo.Version), prefixes, postfixes, transpilers);
		}
		...
		public static DynamicMethod CreatePatchedMethod(MethodBase original, string harmonyInstanceID, List<MethodInfo> prefixes, List<MethodInfo> postfixes, List<MethodInfo> transpilers)
		{
			try
			{
				...
				bool isIl2Cpp = PotatoLoader.UnhollowerSupport.IsGeneratedAssemblyType(original.DeclaringType);
