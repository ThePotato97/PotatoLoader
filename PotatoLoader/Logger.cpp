...
void Logger::Initialize(std::string filepathstr)
{
	...
	std::string logFolderPath = filepathstr + "\\PotatoLogs";
...
void Logger::Log(const char* txt)
{
	...
	Console::Write("[");
	Console::Write("PotatoLoader", ConsoleColor_Magenta);
	Console::Write("] ");
...
void Logger::LogWarning(const char* txt)
{
	if ((MaxWarnings <= 0) || (WarningCount < MaxWarnings))
	{
		...
		if (PotatoLoader::DebugMode || !Console::HideWarnings)
		{
			Console::Write("[PotatoLoader] ", ConsoleColor_Yellow);
...
void Logger::LogError(const char* txt)
{
	if ((MaxErrors <= 0) || (ErrorCount < MaxErrors))
	{
		...
		Console::Write("[PotatoLoader] ", ConsoleColor_Red);
...
void Logger::LogMelonCompatibility(PotatoLoader_Base::MelonCompatibility comp)
{
	LogTimestamp();
	Console::Write("[");
	Console::Write("PotatoLoader", ConsoleColor_Magenta);
	...
	switch (comp)
	{
	case PotatoLoader_Base::MelonCompatibility::UNIVERSAL:
		LogFile << "Universal";
		Console::WriteLine("Universal", ConsoleColor_Cyan);
		break;
	case PotatoLoader_Base::MelonCompatibility::COMPATIBLE:
		LogFile << "Compatible";
		Console::WriteLine("Compatible", ConsoleColor_Green);
		break;
	case PotatoLoader_Base::MelonCompatibility::NOATTRIBUTE:
