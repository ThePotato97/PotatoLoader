...
void Mono::CreateDomain()
{
	if (Domain == NULL)
	{
		...
		mono_runtime_set_main_args(PotatoLoader::CommandLineC, PotatoLoader::CommandLineV);
		Domain = mono_jit_init("PotatoLoader");
...
void Mono::FixDomainBaseDir()
{
	...
	if (!IsOldMono)
		mono_domain_set_config(Domain, PotatoLoader::GamePath, "PotatoLoader");
}
