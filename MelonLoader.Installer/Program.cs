...
namespace MelonLoader.Installer
{
    static class Program
    {
        ...
        private static void Install_Legacy_02(string dirpath, string selectedVersion)
        {
            ...
            string tempfilepath = TempFileCache.CreateFile();
            webClient.DownloadFileAsync(new Uri("https://github.com/ThePotato97/PotatoLoader/releases/download/" + selectedVersion + "/MelonLoader" + (is_02 ? "_" : ".") + (File.Exists(Path.Combine(dirpath, "GameAssembly.dll")) ? "Il2Cpp" : "Mono") + ".zip"), tempfilepath);
            while (webClient.IsBusy) { }
            SetDisplayText("Extracting MelonLoader...");
            Cleanup(dirpath);
            ExtractZip(dirpath, tempfilepath);
        ...
        private static void Install_Legacy_01(string dirpath)
        {
            SetDisplayText("Downloading MelonLoader...");
            string tempfilepath = TempFileCache.CreateFile();
            webClient.DownloadFileAsync(new Uri("https://github.com/ThePotato97/PotatoLoader/releases/download/v0.1.0/MelonLoader.zip"), tempfilepath);
            while (webClient.IsBusy) { }
        ...
        internal static void CheckForUpdates()
        {
            ...
            try
            {
                string response = webClient.DownloadString("https://github.com/ThePotato97/PotatoLoader/raw/master/MelonLoader.Installer/MelonLoader.Installer.csproj");
                ...
                if (!response_p2.Equals(FileVersionInfo.GetVersionInfo(Process.GetCurrentProcess().MainModule.FileName).ProductVersion))
                {
                    ...
                    if (File.Exists(tempfilepath))
                        File.Delete(tempfilepath);
                    webClient.DownloadFileAsync(new Uri("https://github.com/ThePotato97/PotatoLoader/releases/latest/download/MelonLoader.Installer.exe"), tempfilepath);
                    while (webClient.IsBusy) { }
